...
04_score_windows.R -> candidate_amplicons.csv, candidate_amplicon_target_scores.csv, candidate_amplicon_snps.csv
        |
        v
05_select_panel.R -> panel_candidates.csv, panel_target_scores.csv, pareto_panels_by_size.csv, cross_size_pareto_panels.csv, recommended_provisional_panel.csv, panel_size_comparison.csv
        |
...
| `scripts/05_select_panel.R` | Compares small candidate amplicon panels from the stage-04 outputs, evaluates each allowed panel size independently, preserves absolute and target-relative evidence vectors, and reports within-size and cross-size Pareto sets without redesigning primers or the classifier. | `candidate_amplicons.*`, `candidate_amplicon_target_scores.*`, optional `candidate_amplicon_snps.*`, `analysis.panel_selection` | `panel_candidates.csv`, `panel_target_scores.csv`, `pareto_panels_by_size.csv`, `cross_size_pareto_panels.csv`, `recommended_provisional_panel.csv`, `recommended_panel_alternatives.csv`, `panel_size_comparison.csv`, `panel_search_summary.csv` |
...
- Stage 05 now compares candidate amplicon panels size-by-size with transparent evidence vectors and Pareto ranking. It does not design primers or infer clade membership, and the recommended provisional panel remains subject to downstream primer-feasibility review.
