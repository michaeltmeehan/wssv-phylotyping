...
lines <- c(
  ...
  "",
  "This is the stage-03 checkpoint for deciding whether the four-clade simplification is viable before window or primer-panel design. Use it to check whether each configured target clade has enough high-gain discriminatory SNPs to stand on its own.",
  "",
  "Target-clade diagnostics:",
  "",
  table_or_note(
    data$target_clade_diagnostics,
    c(
      "target",
      "target_name",
      "node_id",
    ...
    missing_note = missing_note("target_clade_diagnostics")
  ),
  "",
  "Strongest informative SNPs per target clade:",
  "",
  table_or_note(
    data$target_clade_strongest_snps,
    c(
      "target",
      "target_name",
      "node_id",
    ...
    n = 20L,
    missing_note = missing_note("target_clade_strongest_snps")
  ),
  "",
  ...
  "",
  paste0("- Records read: ", if (is.null(data$partial_classifications)) "missing partial classification output" else nrow(data$partial_classifications)),
  paste0("- Mapped: ", if (is.null(data$partial_classifications) || !"mapped" %in% names(data$partial_classifications)) "missing" else sum(data$partial_classifications$mapped, na.rm = TRUE)),
  paste0("- Unmapped: ", if (is.null(data$partial_classifications) || !"mapped" %in% names(data$partial_classifications)) "missing" else sum(!data$partial_classifications$mapped, na.rm = TRUE)),
  paste0("- Panel-informative records: ", if (is.null(data$partial_classifications_panel) || !"observed_informative_sites" %in% names(data$partial_classifications_panel)) "missing" else sum(data$partial_classifications_panel$mapped & data$partial_classifications_panel$observed_informative_sites > 0L, na.rm = TRUE)),
  paste0("- Opportunistically informative records: ", if (is.null(data$partial_classifications_opportunistic) || !"observed_informative_sites" %in% names(data$partial_classifications_opportunistic)) "missing" else sum(data$partial_classifications_opportunistic$mapped & data$partial_classifications_opportunistic$observed_informative_sites > 0L, na.rm = TRUE)),
  paste0("- Opportunistically assigned nodes: ", if (is.null(data$partial_classifications_opportunistic) || !"status" %in% names(data$partial_classifications_opportunistic)) "missing" else sum(data$partial_classifications_opportunistic$status == "resolved_opportunistic", na.rm = TRUE)),
  "",
  "Partial classification rows:",
  "",
  table_or_note(data$partial_classifications, c("sequence_id", "mapped", "classification_mode", "classification_source", "alignment_start", "alignment_end", "sites_available_in_region", "observed_informative_sites", "supporting_sites", "contradictory_sites", "assigned_node", "assigned_support", "support_margin", "status", "reason"), missing_note = missing_note("partial_classifications")),
  "",
  "Final partial status counts:",
  "",
  table_or_note(status_counts(data$partial_classifications), missing_note = missing_note("partial_classifications")),
  "",
  "Panel status counts:",
  "",
  table_or_note(status_counts(data$partial_classifications_panel), missing_note = missing_note("partial_classifications_panel")),
  "",
  ...
  "",
  "Partial interval diagnostics:",
  "",
  table_or_note(data$partial_region_diagnostics, c("sequence_id", "mapped", "alignment_start", "alignment_end", "sites_available_in_region", "sites_observed", "observed_non_missing_sites"), missing_note = missing_note("partial_region_diagnostics")),
  "",
  "Opportunistic classifications are region-dependent assessments of scored SNPs that happen to fall inside each mapped partial interval. They are not equivalent to the designed-panel classifier or designed-panel validation.",
  "",
