...
- `tree_utils.R`: MCC-tree node, clade, depth, balance, and mask helpers.
- `encoding.R`: alignment and allele encoding helpers.
- `snp_scoring.R`: per-site/per-node scoring functions.
...
During Milestone 0 these files contain comments only.
